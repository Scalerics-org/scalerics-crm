...
# El webhook ahora valida la firma y falla CERRADO: sin secreto rechaza todo.
# Estos tests cubren la extraccion del lead, asi que firman el payload de verdad
# en vez de esquivar la validacion.
_SECRETO = "secreto-de-test"


def _post(app, payload):
    cuerpo = json.dumps(payload).encode()
    ts = str(int(time.time()))
    firma = hmac.new(_SECRETO.encode(), (ts + ".").encode() + cuerpo,
                     hashlib.sha256).hexdigest()
    return app.test_client().post(
        "/api/calendly/webhook", data=cuerpo,
        content_type="application/json",
        headers={"Calendly-Webhook-Signature": f"t={ts},v1={firma}"})
...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CALENDLY_WEBHOOK_SECRET", _SECRETO)
...
def test_new_lead_stores_company_service_and_phone(app):
    ...
    res = _post(app, _payload(FULL_FORM))
...
def test_falls_back_to_person_name_when_no_company(app):
    ...
    res = _post(app, _payload(questions))
