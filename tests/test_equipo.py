...
PANEL = _entre(SRC, "<!-- ======= EQUIPO PANEL ======= -->", "<!-- ======= FIN EQUIPO PANEL ======= -->")
...
# ── criterio 1: una sola pantalla ────────────────────────────────────────────

def test_c1_equipo_abre_una_sola_pantalla_con_las_dos_partes(cli):
    r = cli.get("/")
    assert r.status_code == 200
    pagina = r.get_data(as_text=True)
    assert pagina.count('id="equipo-panel"') == 1
    assert pagina.count('id="nav-equipo"') == 1
    assert "if (name === 'equipo') loadEquipo();" in pagina
    # Arriba el organigrama, abajo el calendario y el detalle, en el mismo panel.
    orden = [PANEL.index(i) for i in ('id="eq-organigrama"', 'id="eq-avisos"',
                                       'id="eq-calendario"', 'id="eq-detalle"')]
    assert orden == sorted(orden)
    assert PANEL.count('class="panel"') == 1
    assert "eqAbrirAusencia()\">Registrar</button>" in PANEL


def test_esta_registrado_en_todos_lados():
    grupo = _entre(HTML, '<div class="nav-section-label">OPERACIÓN</div>',
                   '<div class="nav-section-label">CAPTACIÓN</div>')
    assert grupo.index('id="nav-equipo"') > grupo.index('id="nav-activity"')
    ...
    assert "equipo:'network'" in re.search(r"const NAV_ICONS = \{(.*?)\n\}", HTML, re.S).group(1)
    assert "equipo:'Equipo'" in re.search(r"const NAV_LABELS = \{(.*?)\n\}", HTML, re.S).group(1)
    listas = re.findall(r"const ALL_PANELS = \[([^\]]*)\]", SRC)
    assert len(listas) == 2 and all("'equipo'" in l for l in listas)
    assert "equipo:'Equipo'" in re.search(r"const PANEL_LABELS = \{([^}]*)\}", SRC).group(1)
...
def test_el_simulador_muestra_la_capacidad_leida_de_equipo(cli, tmp_path):
    ...
    assert "leída de Equipo (no editable): 32 h = 40 h base − 8 h de ausencias" in s["capacidad"]
