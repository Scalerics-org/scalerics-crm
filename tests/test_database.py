...
def test_roles_existentes_reciben_el_panel_meta(tmp_path):
    ...
    for nombre, paneles in _roles(db).items():
        # El vendedor de Fidelidad es de afuera: ningún reparto le suma
        # paneles (PR #99, database.py). Se queda con lo que tenía.
        if nombre == "Vendedor Fidelidad":
            assert "meta" not in paneles
            continue
...
def test_migracion_de_panel_es_idempotente(tmp_path):
    ...
    for nombre, paneles in tercera.items():
        # Al vendedor de Fidelidad ningún reparto le suma paneles (PR #99).
        esperado = 0 if nombre == "Vendedor Fidelidad" else 1
        assert paneles.count("meta") == esperado, "'meta' no se puede duplicar en el array"
